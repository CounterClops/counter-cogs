...
class Mod:
    ...
    @_filter.command(name="user", pass_context=True)
    async def filter_user(self, ctx, user: discord.Member):
        if 'Filtered' not in [r.name for r in ctx.message.server.roles]:
            await self.bot.say('The Filtered role doesn\'t exist! Creating it now!')
            try:
                perms = discord.Permissions.none()
                await self.bot.create_role(ctx.message.server, name='Filtered', permissions=perms)
                await self.bot.say("Role created!")
            except discord.Forbidden:
                await self.bot.say("I cannot create a role. Please assign Manage Roles to me!")
        else:
            try:
                role = discord.utils.get(ctx.message.server.roles, name="Filtered")
                await self.bot.add_roles(user, role)
                await self.bot.say("Role given to " + user.name)
            except discord.Forbidden:
                await self.bot.say("Can't assign role to " + user.name)
    ...
    async def check_filter(self, message):
        server = message.server
        if server.id in self.filter.keys():
            role = discord.utils.get(server.roles, name="Filtered")
